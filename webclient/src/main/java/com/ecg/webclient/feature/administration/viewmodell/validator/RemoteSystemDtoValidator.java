...
public class RemoteSystemDtoValidator implements Validator
{
	...
	@Autowired
	public RemoteSystemDtoValidator(RemoteSystemService remoteSystemService)
	{
	...
	public void validate(Object object, Errors errors)
	{
		...
		for (RemoteSystemDto remoteSystem : remoteSystems)
		{
			if (remoteSystem.getId() == -1)
			{
				...
				if (result != null)
				{
					errors.rejectValue("remoteSystems[" + counter + "].name", "remotesystem.rejected.duplicated.name");
				}
			}
			...
			{
				errors.rejectValue("remoteSystems[" + counter + "].loginUrl", "remotesystem.rejected.size.loginUrl");
			}
			...
			if (remoteSystem.getPasswordParameter().isEmpty() || remoteSystem.getPasswordParameter().length() < 1)
			{
